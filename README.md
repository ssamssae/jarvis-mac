...
## v1.2.0: 네스트에서 시작하는 음성 입력과 작업 루틴

이제 설정한 Google Nest에서 다음 문구로 자비스를 시작할 수 있습니다.

| 말할 문구 | 하는 일 |
|---|---|
| “오케이 구글, 보이스 스타토” (`ボイススタート`) | 프로그램 → 컴퓨터를 고른 뒤 본문을 말하고 “엔터”로 기존 세션에 전송 |
| “오케이 구글, 시고토 스타토” (`しごとスタート`) | 설정한 작업 시작 루틴 실행 |
| “오케이 구글, 시고토 오와리” (`しごとおわり`) | 종료 확인 질문을 듣고 Mac 마이크에 “하이” (`はい`)로 한 번 확인 |

자비스를 직접 부를 때도 **“자비스, 보이스 스타토”**로 같은 입력 선택을 시작합니다. 기존 “음성 입력”은 호환용으로 남습니다.

**시작은 네스트, 이후 선택·본문·종료 확인은 Mac 마이크가 받습니다.**
네스트 마이크의 자유 발화 오디오를 넘겨받는 기능은 아닙니다.

업데이트만으로 Google Home 루틴이 자동 등록되지는 않습니다.

1. 아래 설치기로 자비스를 설치·업데이트하고 마이크가 동작하는지 확인합니다.
2. [Matter·Google Home 단계별 설정 안내](docs/nest-matter-setup.md)에 따라 별도 서비스를 설치합니다.
3. 모바일 Google Home에서 작업용·음성 입력용 기기를 각각 페어링합니다. QR 또는 수동 코드를 사용하며 코드는 비공개로 보관합니다.
4. 같은 안내의 일본어 시작 조건과 정확한 기기 ON/OFF 동작으로 루틴을 등록합니다.
5. [세션 입력 설정](docs/voice-session-input.md)에서 프로그램·컴퓨터 대상을 연결한 뒤 짧은 본문으로 실제 수신을 확인합니다.

종료는 질문 재생 성공 후 12초 안의 새 `はい`만 승인합니다. 전원 요청과 실제 기기 전원 상태는 별도 확인합니다.
텔레그램 출처 카드는 지원 브릿지와 별도 수신 기록이 설치된 경우에 표시됩니다.
...
기본 Whisper 인식기에서는 호출 대기 중 0.4~2.5초 길이의 짧은 발화가 호출어로 인식되지 않으면 같은 음성을 영어로 한 번 더 인식합니다. 한국어 결과가 “헤이/hey”로 시작할 때도 기존처럼 재시도합니다. 영어 결과가 “Hey Jarvis” 단독 호출과 정확히 일치해야 호출을 허용하며 질문이나 명령이 붙으면 사용하지 않습니다. 이미 인식된 호출, 호출 후 질문 대기, 별도 `stt_worker`에는 영어 재시도를 적용하지 않습니다.

0.4초 미만의 감지 구간에서 글자·숫자 8자를 넘는 문장이 나오면 짧은 잡음의 과도한 받아쓰기로 판단해 질문으로 넘기지 않습니다. “네”, “취소” 같은 짧은 응답은 유지하며, 걸러진 입력은 남은 질문 대기 시간을 소모하지 않습니다. 이 보수적인 길이 검사는 모든 잡음·오인식을 판별하는 기능은 아닙니다.
...
호출어와 함께 말하거나 호출 후 질문 대기 중인 단독 `취소`, `취소해줘`, `그만`은
로컬에서 대기를 해제하고 `취소했습니다.`라고 응답합니다. 관측된 단독 오인식
`치즈소`·`치치소`·`치솔`도 이 문맥에서만 취소로 처리합니다. `치즈 소스 만드는 법`,
`취소 방법 알려줘` 같은 문장 전체를 취소로 바꾸지 않습니다. 모델과 가전 명령은
호출하지 않으며, 작업 종료 확인 대기도 함께 해제합니다. 이 처리는 음성 모델의
정확도 자체를 바꾸는 것은 아닙니다. 답변 생성·재생 중 마이크가 닫히는 기존 구조는
유지되므로 재생 중 끼어들어 중단하는 기능은 아직 지원하지 않습니다.

Cast의 수신기/미디어 상태 조회가 일시적으로 실패하면 읽기 요청만 150ms 후 한 번
재시도합니다. 재생 중 상태 조회에도 적용하지만 LOAD나 가전 명령을 다시 실행하지
